...
>
> ![Untitled2](./CSEE%204119%20Computer%20Networks.assets/Untitled2.png)
>
...
>
> ![Untitled1](./CSEE%204119%20Computer%20Networks.assets/Untitled1.png)
>
> ![Untitled](./CSEE%204119%20Computer%20Networks.assets/Untitled.png)
>
...
### Destination-based forwarding - **Forwarding table**
...
![image-20221122164835325](https://images.wu.engineer/images/2022/11/22/image-20221122164835325.png)
...
![image-20221121194845342](https://images.wu.engineer/images/2022/11/21/image-20221121194845342.png)
...
![image-20221121234614345](https://images.wu.engineer/images/2022/11/21/image-20221121234614345.png)
...
**scale: **with billions of desitinations
...
- gateway router: at “edge” of an own AS, has links to routers in other AS
...
![image-20221122195612689](https://images.wu.engineer/images/2022/11/22/image-20221122195612689.png)
...
1. learn which dentations are reachable through AS2, which through AS3
2. propagate this reachability info to all routers in AS1

![image-20221122200447284](https://images.wu.engineer/images/2022/11/22/image-20221122200447284.png)

### Intra-AS routing

- Also know as **interior gateway protocols (IGP)** 内部网关协议
...
### Supplementary material: ELEN30024 Data Networking - BGP route

![Untitled](https://images.wu.engineer/images/2022/11/22/Untitled-1669164806630-1.png)

> AS代表自治系统 每个自治系统里的三个router使用相同的内部自治系统路由协议/算法(Intra-autonomous system router protocol/algorithm) R_X, R_T, R_V为内部路由（internal router），其只在自己的内部自治系统中与路由链接。其余的为网关路由（gateway router）

![Untitled](https://images.wu.engineer/images/2022/11/22/Untitled-1669164822453-3.png)

> R_V路由接口有两台计算机，被称为子网B(Subnet with prefix ‘B’) 子网B如何与R_X建立链接？

> R_U和R_W通过**内部边界网关协议（internal BGP(iBGP)）\**知晓子网B的存在，因为他们在同一个AS中 R_Z作为网关路由，通过R_W的IP地址“c”知晓子网B在AS3中。R_Z和R_W的交流通过\**外部边界网关协议（external BGP(eBGP)）** **“IPc；AS3；B“被称为BGP线路（BGP route）** **IPc被称为下跳（Next-Hop）** **AS3称为AS-PATH**

R_Z passes the same message of “Subnet B is in AS3 through IP address of IPc” to R_X and R_Y

R_X receives this message from **Interface 2 via R_Z to R_X**

![Untitled](https://images.wu.engineer/images/2022/11/22/Untitled-1669164842925-5.png)

> 同时，R_X也可以通过AS3→AS2→AS1到达子网B

- Gateway router R_U sends R_S in AS2 a message “subnet B is in AS3 through IP address of **IPb**” via external BGP (eBGP)
- R_S send the same message to R_Q and R_T via internal BGP (iBGP)
- Gateway router R_Q sends R_Y in AS1 a message “Subnet B is in AS3 through AS2 with IP address of **IPa**” via external BGP (eBGP)
- R_Y send the same message to R_X and R_Z via internal BGP (iBGP)
- R_X receives the message from link interface 1

> 现在R_X有两条BGP线路可以选择

![Untitled](https://images.wu.engineer/images/2022/11/22/Untitled-1669164865798-7.png)

1. Blindly choose the **shortest AS-PATH** when there is **no information on the cost** for these routes

   > 当没有相关路由成本时，选择AS-PATH最短的线路

2. Choose the lowest cost route
...
- two-level hierarchy: local area, backbone
  ...
  - each nodes has detailed area topology; only know direction (shortest) to nets in other areas
...
![image-20221122201728551](https://images.wu.engineer/images/2022/11/22/image-20221122201728551.png)
...
- Policy-based routing:
  - gateway receiving route advertisement uses **import policy** to accept/decline path (e.g., never router thorough ASY)
  - **route selection policy** to pick best path when multiple exist
    - NOT NECESSARILY shortest path
  - **export policy** determines which (if any) neighbouring ASes to advertise best path to
    - NOT NECESSARILY all paths to all neighbours
    - CAN be different to different neighbours

### eBGP, iBGP connections

![image-20221122203814572](https://images.wu.engineer/images/2022/11/22/image-20221122203814572.png)
...
![image-20221122211600478](https://images.wu.engineer/images/2022/11/22/image-20221122211600478.png) 
...
![image-20221122211946039](https://images.wu.engineer/images/2022/11/22/image-20221122211946039.png)

- AS2 router 2c receives path advertisement AS3,X (via eBGP) from AS3 router 3a
- Based on AS2 policy, AS2 router 2c accepts path AS3,X, propagates (via iBGP) to all AS2 routers
- Based on AS2 policy, AS2 router 2a advertises (via eBGP) path AS2,AS3,X to AS1 router 1c

![image-20221122212132165](https://images.wu.engineer/images/2022/11/22/image-20221122212132165.png)

- AS1 gateway router 1c learns path AS2,AS3,X from router 2a
- AS1 gateway router 1c learns oath AS3,X from 3a
- Based on policy, AS1 gateway router 1c chooses path AS3,X, and advertise path within AS1 via iBGP

### BGP NEXT-HOP v1

![image-20221122212342551](https://images.wu.engineer/images/2022/11/22/image-20221122212342551.png)

### GBP, OSPF, forwarding table entries

Q: How does router set forwarding table entry to distant prefix

![image-20221122212431171](https://images.wu.engineer/images/2022/11/22/image-20221122212431171.png)

![image-20221122212441155](https://images.wu.engineer/images/2022/11/22/image-20221122212441155.png)

![image-20221122212524591](https://images.wu.engineer/images/2022/11/22/image-20221122212524591.png)

![image-20221122212530420](https://images.wu.engineer/images/2022/11/22/image-20221122212530420.png)

### BGP NEXT-HOP v2

![image-20221122212503089](https://images.wu.engineer/images/2022/11/22/image-20221122212503089.png)
